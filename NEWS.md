...
Changes in Version 0.5-6
   ...
   - insert_ref() and Rd macro \insertRef should now work ok in the presence of
     percent encoded symbols in URL field of a BibTeX entry.
     Closes issue "Unexpected END_OF_INPUT error (URL parsing?)", see
     https://github.com/GeoBosh/Rdpack/issues/3,
     raised by jdnewmil (https://github.com/jdnewmil).

   - get_bibentries() now takes care of percent encoded symbols in URLs.
     It now returns an object from class "bibentryRd", which inherits from
     ...
     also reported by by jdnewmil in issue#3 referenced above. 
...
Changes in Version 0.5-5 (CRAN)
   ...
   - new function ereprompt() (`e' for edit) - opens reprompt() with suitable
   ...
   - README.* now get links to https://github.com/GeoBosh/georgisemacs
     for an emacs function to reprompt() the Rd file at point.
...
Changes in Version 0.5-4.9000
   ...
   - new RStudio add-in 'Reprompt', contributed by Duncan Murdoch.
     ...
     reprompt(). If the file is as R source file, it looks for the help page of
     a selected object name and works on it if found, otherwise creates one.
