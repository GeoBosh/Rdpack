...
\label{sec:org766ffef}

\subsection{Warning from 'R CMD build'}
\label{sec:org8906f2e}
...
included in REFERENCES.bib\footnote{Thanks to Michael Dewey for suggesting the discussion of this.\label{org76ee4c2}}. Only do this for entries that do not render properly or
...
Users of 'devtools' can use \code{viewRd()} in place of \code{help()} to view Rd sources\textsuperscript{\ref{org76ee4c2}}.
\end{document}
