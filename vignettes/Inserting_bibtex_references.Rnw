...
Note the doubled \texttt{@} in the last example: the first \texttt{@} switches the argument
to free-text mode, the second is the prefix that marks \texttt{parseRd} as a key
(\texttt{chapter 3} has no \texttt{@}, so it stays as text).

If using \pkg{roxygen2}, note that \verb|@@| in an \texttt{.R} source file is replaced with a
single \texttt{@} in the corresponding \texttt{.Rd} documentation file. Hence the
key-plus-free-text form must be written as \verb|@@@@parseRd, chapter 3|, such that
\pkg{roxygen2} writes \verb|@@parseRd, chapter 3|.
