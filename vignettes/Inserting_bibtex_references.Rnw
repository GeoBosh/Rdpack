...
\label{sec:org89c951a}

\subsection{Warning from 'R CMD build'}
\label{sec:orgab7498b}
...
included in REFERENCES.bib\footnote{Thanks to Michael Dewey for suggesting the discussion of this.\label{orge2ba763}}. Only do this for entries that do not render properly or
...
Users of 'devtools' can use \code{viewRd()} in place of \code{help()} to view Rd sources\textsuperscript{\ref{orge2ba763}}.
\end{document}
