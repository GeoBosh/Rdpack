...
\label{sec:org4952be3}

\subsection{Warning from 'R CMD build'}
\label{sec:orgf964b45}
...
included in REFERENCES.bib\footnote{Thanks to Michael Dewey for suggesting the discussion of this.\label{orgc49c0a2}}. Only do this for entries that do not render properly or
...
Users of 'devtools' can use \code{viewRd()} in place of \code{help()} to view Rd sources\textsuperscript{\ref{orgc49c0a2}}.
\end{document}
