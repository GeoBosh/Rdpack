...
From version 0.7 of \pkg{Rdpack}\footnote{Actually, they were introduced in the development
  version 0.6-1, but 0.7 is the first version with them released on CRAN.}, additional Rd
macros are available for citations.  They can be used in both Rd and roxygen2 documentation.
If you are using these, it will be prudent to require at least this version of Rdpack in
the ``Imports:'' directive in file DESCRIPTION: \verb+Rdpack (>= 0.7)+.

\subsection{Macros fof citations}
\label{sec:macros-citations}

\verb+\insertCite{key}{package}+ cites the key and records it for use by
\verb+\insertAllCited{}+, see below. \code{key} can contain more keys separated by commas.
...
\begin{tabular}{l|l}
  ...
  \verb+\insertCite{parseRd}{Rdpack}+ &  
...
\begin{tabular}{l|l}
  Documentation source & rendered \\ \hline
  \verb+\insertCite{parseRd;textual}{Rdpack}+ &  
            \Sexpr{insert_citeOnly("parseRd;textual", "Rdpack")}     \\
  \verb+\insertCite{Rpack:bibtex;textual}{Rdpack}+   & 
            \Sexpr{insert_citeOnly("Rpack:bibtex;textual", "Rdpack")} \\
  \verb+\insertCite{parseRd,Rpack:bibtex;textual}{Rdpack}+ &
...
The macro \verb+\insertNoCite{key}{package}+ records one or more references for
\verb+\insertAllCited+ but does not cite it. Setting \verb+key+ to \verb+*+ will record all
references from the specified package. For example, \verb+\insertNoCite{R}{bibtex}+ records
the reference whose key is \verb+R+, while \verb+\insertNoCite{*}{utils}+ records all
references from package ``utils'' for inclusion by \verb+\insertAllCited+.

To mix the citations with other text, such as ``see also'' and ``chapter 3'', write the list
of keys as a free text, starting it with the symbol \verb+@+ and prefixing each key with it.
...
\verb+\insertCiteOnly{key}{package}+ is as \verb+\insertCite+ but does not record the key 
for the list of references assembled by \verb+\insertAllCited+.

\subsection{Automatically generating lists of references}
\label{sec:autom-gener-list}

The macro \verb+\insertAllCited{}+ can be used to insert all references cited with
\verb+\insertCite+ or \verb+\insertNoCite+. A natural place to put this macro is the
