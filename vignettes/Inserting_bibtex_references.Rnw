...
\label{sec:org785d773}

\subsection{Warning from 'R CMD build'}
\label{sec:org6583efe}
...
included in REFERENCES.bib\footnote{Thanks to Michael Dewey for suggesting the discussion of this.\label{org703d245}}. Only do this for entries that do not render properly or
...
Users of 'devtools' can use \code{viewRd()} in place of \code{help()} to view Rd sources\textsuperscript{\ref{org703d245}}.
\end{document}
