...
\label{sec:org1f5915d}

\subsection{Warning from 'R CMD build'}
\label{sec:orga0d6c1e}
...
included in REFERENCES.bib\footnote{Thanks to Michael Dewey for suggesting the discussion of this.\label{org6915713}}. Only do this for entries that do not render properly or
...
Users of 'devtools' can use \code{viewRd()} in place of \code{help()} to view Rd sources\textsuperscript{\ref{org6915713}}.
\end{document}
