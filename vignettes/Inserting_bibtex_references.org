...
| \Sexpr{insert_citeOnly("@@parseRd, chapter 3", "Rdpack")} |

Note the doubled =@= in the last example: the first =@= switches the argument to free-text
mode, the second is the prefix that marks =parseRd= as a key (=chapter 3= has no =@=, so it
stays as text).

If using \pkg{roxygen2}, note that =@@= in an =.R= source file is replaced with a single =@=
in the corresponding =.Rd= documentation file. Hence the key-plus-free-text form must be
written as =@@@@parseRd, chapter 3=, such that \pkg{roxygen2} writes =@@parseRd, chapter 3=.
