...
\begin{verbatim}
    ...
    ##: 6
\end{verbatim}
The argument of \texttt{\textbackslash{}printExample} must be on a single line with released versions of R\footnote{This limitation has been lifted in \texttt{R-devel} starting from June 2018 and in \texttt{R-3.6.0}.
In earlier versions of \texttt{R} the lines after the first are ignored without warnings or
errors. This should not be a big concern if you already require \texttt{R >= 3.6.0} or can live
with somewhat inferior documentation in older versions or \texttt{R}.}. 
...
r75388.)}. A small example package
